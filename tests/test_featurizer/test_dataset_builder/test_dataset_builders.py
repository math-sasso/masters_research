...
def occurance_dataset_builder(mock_species_shapefile_path, processed_raster_paths_list):
    occ_dataset_builder = OccurrancesDatasetBuilder(
        processed_raster_paths_list
    )
...
@pytest.fixture
def pseudo_absense_dataset_builder(occurance_dataset_builder, mock_mask_raster_path, mock_environment_dirpath):
    ...
    psa_dataset_builder = PseudoAbsensesDatasetBuilder(
            ...
            stacked_raster_coverages_path= mock_environment_dirpath / "environment_stack.npy",
    )

    occ_df = occurance_dataset_builder.get_dataset()
    psa_dataset_builder.build(
            occurrence_df= occ_df, number_pseudo_absenses=len(occ_df)
    )
