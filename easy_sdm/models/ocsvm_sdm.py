...
class SDM:
  ...
  """
    ...
    """
        ...
        self.land_reference, _, _, _, _, _ = self.raster_utils.get_raster_infos(
            land_reference_path
        )
        np.random.seed(self.seed)
        self.stacked_raster_coverages = utils_methods.retrieve_data_from_np_array(
            stacked_rasters_path
        )
        ...
        idx = np.where(
            self.land_reference == self.raster_utils.positive_mask_val
    ...
    def fit(self, species_bunch):
        ...
        train_cover_std = (
            species_bunch["raster_data_train"] - self.mean_vars
        ) / self.std_vars
        train_cover_std[
    ...
    def predict_land(self, clf):
        ...
        raster_coverages_land = self.stacked_raster_coverages[
            :, self.idx_X, self.idx_Y
        ].T
        for k in range(raster_coverages_land.shape[1]):
            raster_coverages_land[:, k][
                raster_coverages_land[:, k] <= self.raster_utils.no_data_val
            ] = self.mean_vars[k]
        ...
        Z = np.ones(
            (stacked_raster_coverages_shape[1], stacked_raster_coverages_shape[2]),
            dtype=np.float32,
        )
        # Z *= global_pred.min()
        # Z *=-1 #This will be necessary to set points outside map to the minimum
        Z *= (
            self.raster_utils.no_data_val
        ...
        Z[
            self.land_reference == self.raster_utils.no_data_val
        ] = self.raster_utils.no_data_val
    ...
    def predict_test_occurences(self, species_bunch, clf):
        ...
        scaled_species_raster_test = (
            species_bunch["raster_data_test"] - self.mean_vars
        ) / self.std_vars
    ...
    def perform_K_folder_preidction(
        self,
        species_occurence_path: str,
        specie_shp_path: str,
        list_raster_files: List,
        K: int,
    ):
        ...
        coordinates = np.array(
            (np.array(species_gdf["LATITUDE"]), np.array(species_gdf["LONGITUDE"]))
        ).T
        species_raster_data = self.utils_methods.retrieve_data_from_np_array(
            species_occurence_path
        )
        ...
        for i, (train_index, test_index) in enumerate(kf.split(species_raster_data)):
            ...
            kfold_path = os.path.join(
                self.output_base_folder, species_name, f"KFold{i+1}"
            )
            self.utils_methods.create_folder_structure(kfold_path)

            species_raster_data_train, species_raster_data_test = (
                species_raster_data[train_index],
                species_raster_data[test_index],
            )
            coords_train, coords_test = (
                coordinates[train_index],
            ...
            self.utils_methods.save_nparray_to_folder(
                pred_test, kfold_path, "Test_Prediction"
            )
            del pred_test
            # save coords_train
            self.utils_methods.save_nparray_to_folder(
                species_bunch["coords_train"], kfold_path, "Coords_Train"
            )
            # save_coords_test
            self.utils_methods.save_nparray_to_folder(
                species_bunch["coords_test"], kfold_path, "Coords_Test"
            )
            # raster_data_train
            self.utils_methods.save_nparray_to_folder(
                species_bunch["raster_data_train"],
                kfold_path,
                "Species_Raster_Data_Train",
            )
            # raster_data_test
            self.utils_methods.save_nparray_to_folder(
                species_bunch["raster_data_test"],
                kfold_path,
