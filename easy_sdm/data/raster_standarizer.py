...
class RasterCliper:
    ...
    def __data_conversions(self, source_raster:rasterio.io.DatasetReader, cropped_raster_matrix:np.ndarray):
        map_limits = configs["maps"]["brazil_limits_with_security"]
        ...
        if result_profile["nodata"] != configs["maps"]["no_data_val"]:
            cropped_data = np.where(cropped_data < configs["maps"]["no_data_val"], configs["maps"]["no_data_val"], cropped_data)
    ...
    def clip_and_save(self, source_raster:rasterio.io.DatasetReader, output_path: str):
        result_profile = self.clip(source_raster)
        with rasterio.open(output_path, "w", **result_profile) as dst:
            dst.write(output_path, 1)
