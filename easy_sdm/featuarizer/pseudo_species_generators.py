...
class RSEPPseudoSpeciesGenerator(BasePseudoSpeciesGenerator):
    ...
    def __get_decision_points(self):
        ...
        """
        Z = np.ones(
            (self.stacked_raster_coverages.shape[1], self.stacked_raster_coverages.shape[2]),
            dtype=np.float32,
        )
        Z *= self.configs["mask"][
            "negative_mask_val"
        ...
        inside_country_values = self.stacked_raster_coverages[:,self.inside_mask_idx[1], self.inside_mask_idx[0]].T
        Z[self.inside_mask_idx[0], self.inside_mask_idx[1]] = self.ocsvm.predict(inside_country_values)
    ...
    def generate(self, number_pseudo_absenses: int):
        ...
        for _ in range(number_pseudo_absenses):
            ...
            pseudo_absenses_df = pseudo_absenses_df.append(pseudo_absense_row,ignore_index=True)
